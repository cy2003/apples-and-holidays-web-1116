...
def all_supplies_in_holidays(holiday_hash)
  ...
  puts "#{season[0].to_s.capitalize}:"
    season[1].each do |holiday|
      puts "  #{holiday[0].to_s.gsub('_',' ').split.map{|name| name.capitalize}.join(' ')}: #{holiday[1].join(', ')}"
    # puts "  #{holiday[0].to_s.gsub('_',' ').split.map(&:capitalize).join(' ')}: #{holiday[1].join(', ')}" Can use the &: as well
    end
  end
